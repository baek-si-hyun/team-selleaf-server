...
const addPaginationEvent = (pageInfo) => {
    ...
    if (pageInfo.page > 1) {
        ...
        pageCountPrev.addEventListener("click", async () => {
            await lectureService.getList(keyword, --page, createHTML.showList);
        });
    }
...
const createHTML = (() => {
    ...
    const showList = async (lectures) => {
        // HTML 코드를 담을 빈 문자열
        let text = ``;
        // 페이지네이션+전체 신고 내역 수를 담은 dict 데이터를 배열에서 분리해서 pageInfo에 할당
        let pageInfo = lectures.pop()
        ...
        if (lectures.length === 0){
            text += `
                <div class="nothing">
                    ...
                    <p class="nothing">개설된 강의가 없습니다.</p>
                </div>
            `
        }
            ...
            for (let lecture of lectures){
                ...
                text += `
                  <li class="list-content ${lecture.id}">
                    ...
                    <a class="class-info-wrap" href="/managers-page/lecture/review/?id=${lecture.id}" target="_blank">
...
const callFirstLectureList = () => {
  ...
  lectureService.getList(keyword, page, createHTML.showList);
  allCheckBox.checked = false;
  deleteBtn.disabled = true;
}

// 페이지 열렸을 때 사용
callFirstLectureList();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  confirmButton.addEventListener("click", async () => {
    ...
    await lectureService.deleteLectures(deleteIds);
...
const handleCancelClick = async () => {
    ...
    page = 1 // 페이지 1로 초기화
    await lectureService.getEntryList(keyword, page, createHTML.showList) // 신고 목록을 다시 불러옴
}
