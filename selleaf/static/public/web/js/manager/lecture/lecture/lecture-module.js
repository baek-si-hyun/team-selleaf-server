...
const lectureService = (() => {
    ...
    const getList = async (keyword, page, callback) => {
        // API에 데이터 요청
        const response = await fetch (`/service-admin/lecture-list/?keyword=${keyword}&page=${page}`);
    ...
    const deleteLectures = async (lectureIds) => {
        await fetch(`/service-admin/lecture/delete/${lectureIds}`, {
            method: 'PATCH',
    ...
    const getReviews = async (lectureId, keyword, page, callback) => {
        const response = await fetch(`/service-admin/lecture/review-list/?lectureId=${lectureId}&keyword=${keyword}&page=${page}`);
    ...
    const deleteReviews = async (lectureIds) => {
        await fetch(`/service-admin/lecture/review/delete/${lectureIds}`, {
            method: 'delete',
            headers: {'X-CSRFToken': csrf_token}
        });
    }
    ...
    const getTrainees = async (lectureId, keyword, page, callback) => {
        const response = await fetch(`/service-admin/lecture/trainees-list/?lectureId=${lectureId}&keyword=${keyword}&page=${page}`);
