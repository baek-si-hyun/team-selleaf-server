...
const teacherService = (() => {
    ...
    const getList = async (keyword, page, callback) => {
        // API에 데이터 요청
        const response = await fetch (`/managers-page/teacher-list/?keyword=${keyword}&page=${page}`);
    ...
    const getEntryList = async (keyword, page, callback) => {
        // API에 데이터 요청
        const response = await fetch (`/managers-page/teacher-entry-list/?keyword=${keyword}&page=${page}`);
    ...
    const approveTeachers = async (teacherIds) => {
        await fetch(`/managers-page/teacher-approve/${teacherIds}`, {
            method: 'PATCH',
    ...
    const deleteTeachers = async (teacherIds) => {
        await fetch(`/managers-page/teacher-delete/${teacherIds}`, {
            method: 'PATCH',
            headers: {'X-CSRFToken': csrf_token}
        });
    }
