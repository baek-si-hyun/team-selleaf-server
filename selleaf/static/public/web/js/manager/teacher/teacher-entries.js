...
const createHTML = (() => {
    ...
    const showList = async (teachers) => {
        // HTML 코드를 담을 빈 문자열
        let text = ``;
        // 페이지네이션+전체 신고 내역 수를 담은 dict 데이터를 배열에서 분리해서 pageInfo에 할당
        let pageInfo = teachers.pop()
        ...
        if (teachers.length === 0){
            text += `
                <div class="nothing">
                    ...
                    <p class="nothing">강사 신청자가 없습니다.</p>
                </div>
            `
        }
...
const callFirstEntryList = () => {
  teacherService.getEntryList(keyword, page, createHTML.showList);
...
inputField.addEventListener("keyup", async (e) => {
    ...
    if (e.keyCode === 13) {
        ...
        await teacherService.getEntryList(keyword, page, createHTML.showList)
    }
})
...
const handleCancelClick = async () => {
    ...
    page = 1 // 페이지 1로 초기화
    await teacherService.getEntryList(keyword, page, createHTML.showList) // 신고 목록을 다시 불러옴
}
