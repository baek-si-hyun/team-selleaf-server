...
const addPaginationEvent = (pageInfo) => {
    ...
    if (pageInfo.page > 1) {
        ...
        pageCountPrev.addEventListener("click", async () => {
            await reportService.getTradeReports(keyword, --page, createHTML.showList);
        });
    }
...
const callFirstTradeReportsList = () => {
  reportService.getTradeReports(keyword, page, createHTML.showList);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  confirmButton.addEventListener("click", async () => {
    ...
    await reportService.deleteTradeReports(deleteIds);

    // 다시 리스트 요청 - 키워드는 유지
    reportService.getTradeReports(keyword, page=1, createHTML.showList);
    allCheckBox.checked = false;
    deleteBtn.disabled = true;
    approveBtn.disabled = true;
  });
  ...
  approveConfirmButton.addEventListener('click', async () => {
    ...
    await reportService.approveTradeReports(approveIds);
...
const handleCancelClick = async () => {
    ...
    page = 1 // 페이지 1로 초기화
    await reportService.getTradeReports(keyword, page, createHTML.showList) // 신고 목록을 다시 불러옴
}
