        ...
        if (teachers.length === 0){
            text += `
                <div class="nothing">
                    ...
                    <p class="nothing">강사 리스트가 없습니다.</p>
                </div>
            `
        }
...
const callFirstTeacherList = () => {
  teacherService.getList(keyword, page, createHTML.showList);
...
inputField.addEventListener("keyup", async (e) => {
    ...
    if (e.keyCode === 13) {
        ...
        await teacherService.getList(keyword, page, createHTML.showList)
    }
})
...
const handleCancelClick = async () => {
    ...
    page = 1 // 페이지 1로 초기화
    await teacherService.getList(keyword, page, createHTML.showList) // 신고 목록을 다시 불러옴
}
