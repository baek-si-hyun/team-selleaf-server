...
const addPaginationEvent = (pageInfo) => {
    ...
    if (pageInfo.page > 1) {
        ...
        pageCountPrev.addEventListener("click", async () => {
            await paymentService.getPaymentList(keyword, --page, createHTML.showList);
        });
    }
...
// 받아온 결제 사항들을 뿌려주는 함수
// 전체 결제 수 표시할 태그, 결제 내역 리스트 뿌릴 태그, 페이지 버튼 감싸는 태그
...
// HTML 코드 생성 모듈
const createHTML = (() => {
    // 결제 내역 리스트 표시 - API 요청 데이터 전체(결제 내역 정보+페이지네이션 변수, 배열)를 인자로 받음
    const showList = async (payments) => {
        // HTML 코드를 담을 빈 문자열
        let text = ``;
        // 페이지네이션+전체 결제 내역 수를 담은 dict 데이터를 배열에서 분리해서 pageInfo에 할당
        let pageInfo = payments.pop()
        // 받아온 dict 데이터에서 전체 결제 내역 수를 화면에 표시
        ...
        // 결제 내역 정보가 없으면 내역 없음 표시
        if (payments.length === 0){
            text += `
                <div class="nothing">
                    <img src="/static/public/web/images/manager/nothing.png" class="nothing"/>
                    <p class="nothing">결제 내역이 없습니다.</p>
                </div>
            `
        }
        // 결제 내역이 하나라도 있다면 아래 코드로 결제 내역 리스트 생성
        else {
            // 결제 내역 각각을 HTML 코드에 담아서 text에 추가
            for (let payment of payments){
                ...
                if (payment.payment_status === 1) {
                    paymentStatus = '결제 완료';
                    paymentDate = payment.created_date;
                ...
                // 수강료에 서식 적용
                let lecturePrice = priceFormat(payment.payment_price);

                // 총 구매액 계산 후 서식 적용
                let totalPrice = priceFormat(payment.payment_price * payment.payment_quantity);

                //
                if (payment.payment_kit) {
                  paymentKit = payment.payment_kit;
                }

                text += `
                    <li class="list-content">
                    <div class="payment-info-wrap">
                      <div class="payment-info">${payment.payment_member}</div>
                      <div class="payment-info">${payment.payment_lecture}</div>
                      <div class="payment-info">${paymentKit}</div>
                      <div class="payment-info">${lecturePrice}원</div>
                      <div class="payment-info">${payment.payment_quantity}</div>
                      <div class="payment-info">${totalPrice}원</div>
                      <div class="payment-info">${payment.payment_address}</div>
                      <div class="payment-info">${paymentStatus}</div>
                      <div class="payment-info">${paymentDate}</div>
                    </div>
                  </li>
                `
            }
        }
        // 완성한 결제 내역 내역을 화면에 표시
    ...
    const showPagination = (pageInfo) => {
        const totalCount = pageInfo.totalCount; // 전체 결제 내역 수
        ...
        const realEnd = pageInfo.realEnd; // 결제 내역 리스트의 맨 마지막 페이지

        // 페이지네이션 HTML 코드를 담을 빈 문자열
        let text = ``;
        // 결제 내역이 하나도 없다면 페이지네이션 버튼 표시 안 함
        if (totalCount === 0) {
            text = ``;
        }
        // 결제 내역이 하나라도 있다면 아래 코드로 페이지네이션 버튼 추가
        else{
    ...
    // 결제 내역 내역 표시 기능만 반환 - 페이지네이션 버튼 표시도 같이 실행
    // 페이지네이션 버튼만 화면에 표시할 이유가 없기 때문
    return {showList: showList}
})()

paymentService.getPaymentList(keyword, page, createHTML.showList);
...
const handleCancelClick = async () => {
    ...
    page = 1 // 페이지 1로 초기화
    await paymentService.getPaymentList(keyword, page, createHTML.showList) // 결제 내역 목록을 다시 불러옴
}
